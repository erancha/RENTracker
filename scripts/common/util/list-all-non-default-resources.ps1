...
try {
    ...
    #======== Eventbridge buses =============================================================
    $eventBuses = aws events list-event-buses --region $region --query "EventBuses[?Name!='default'].{Name:Name,Arn:Arn}" --output json | ConvertFrom-Json
    foreach ($bus in $eventBuses) {
        $nonDefaultResources += "EventBridge Bus: $($bus.Name.PadRight(20)) : $($bus.Arn)"
    }
